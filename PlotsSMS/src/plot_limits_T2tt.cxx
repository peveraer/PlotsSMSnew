...
int main(){
  ...
  TString pp_gluglu("pp #rightarrow #tilde{t}#kern[0.3]{#bar{#tilde{t}}}");
  TString basetitle(pp_gluglu+",  #tilde{t} #rightarrow ");
  ...
  TString folder("root/limits_2017/");
  vector<model_limits> models;

  ///////////////////////////////    Defining T2tt plot    /////////////////////////////////
 
  models.push_back(model_limits("T2tt", basetitle+"t#kern[0.4]{"+lsp+"}"));
  models.back().add("SUS-16-033, 0-lep ("+mht+")", folder+"T2tt_RA2.root", 
  		    cSus15002, "ObsLim2", "ExpLim2");
  models.back().add("SUS-16-036, 0-lep ("+mt2+")", folder+"T2tt_MT2.root", 
		    cSus15003, "ObsLim", "ExpLim");
 // models.back().add("SUS-16-016, 0-lep ("+aT+"), 35.9 fb^{-1}", folder+"T2tt_alphaT_v2.root", 
 ...
 //		    cSus15004_1l, "Obs_T1tttt_MuMultiJet_EleMultiJet", "Exp_T1tttt_MuMultiJet_EleMultiJet");
  ...
  models.back().add("SUS-16-049, 0-lep stop", folder+"T2tt_A.root", 
		    cSus16007A, "graph_smoothed_Obs", "graph_smoothed_Exp");
  models.back().add("SUS-16-051, 1-lep stop", folder+"T2tt_1l.root",
		    cSus16002, "gObs", "gExp");
  models.back().add("SUS-17-001, 2-lep stop", folder+"T2tt_2l.root", 
    		    cSus15004, "contour_obs", "contour_exp");
  //  models.back().add("SUS-16-028, 1-lep stop, 35.9 fb^{-1}", folder+"T2tt_1l.root", 
  ...
  //  models.push_back(model_limits("T2ttdedicated", basetitle+"t#kern[0.4]{"+lsp+"}"));
  //  models.back().add("SUS-16-014, 0-lep ("+mht+"), 35.9 fb^{-1}", folder+"T2tt_RA2.root", 
  //		    cSus15002, "ObsLim", "ExpLim");
  // models.back().add("SUS-16-015, 0-lep ("+mt2+"), 35.9 fb^{-1}", folder+"T2tt_MT2.root", 
  //		    cSus15003, "gr_obs_smoothed", "gr_exp_smoothed");
  // models.back().add("SUS-16-016, 0-lep ("+aT+"), 35.9 fb^{-1}", folder+"T2tt_alphaT_v2.root", 
  //		    cSus15004, "observed_total", "expected_total");
 //  models.back().add("SUS-15-004, 1-lep (Razor), 2.1 fb^{-1}", folder+"t1tbqq_sus15_004.root", 
 //		    cSus15004_1l, "Obs_T1tttt_MuMultiJet_EleMultiJet", "Exp_T1tttt_MuMultiJet_EleMultiJet");
  //  models.back().add("SUS-16-029, 0-lep stop, 35.9 fb^{-1}", folder+"T2tt_A.root", 
  //		    cSus16007A, "graph_smoothed_Obs", "graph_smoothed_Exp");
//  models.back().add("SUS-16-030, 0-lep (top tag), 35.9 fb^{-1}", folder+"T2tt_alpha.root", 
  ...
  //   models.back().add("", folder+"T2tt_alpha.root", 
  // 		    cSus16007alpha, "combined_obsExclOneTimesProspino_base_BR100pct", "combined_expExclOneTimesProspino_extra_BR100pct");
  //  models.back().add("Combination 0-lep and 1-lep stop, 35.9 fb^{-1}", folder+"comb.root", 
  //  		    cComb, "gObs", "gExp");
  //////////////////////////////////////////////////////////////////////////////////////// 
  //////////////////////////////////    Making plots    //////////////////////////////////
  //////////////////////////////////////////////////////////////////////////////////////// 
  ...
  for(size_t imodel(0); imodel < models.size(); imodel++){
    ...
    float Xmin(100), Xmax(1200), Ymin(0), Ymax(900), glu_lsp;
    if(ncurves<10) Ymax=800;
    ...
    for(size_t file(0); file < ncurves; file++){
      ...
      //      if(mod.labels[file].Length()>2) obs[file]->Draw("f same");

      TString obsname("obs"); obsname += imodel; obsname += file;
      obs[file]->SetName(obsname);
    } 
    //obs[ncurves-1]->SetFillColor(kWhite);
    //    obs[ncurves-1]->Draw("f same");
    ...
    baseleg.Draw();
    obsleg.Draw();
    legX = lMargin+0.03; legY = 1-tMargin-cmsH-0.015; //legY = 1-tMargin-cmsH-0.035;
    legW = 0.13; 
    //    legH = legLineH * ncurves * mod.legScale;
    int numberlines=4;
    if(ncurves>10) numberlines=7;
    legH = legLineH * numberlines * mod.legScale;
...
void addLabelsTitle(float lMargin, float tMargin, float rMargin, TString title){
  ...
  //  label.DrawLatex(1-rMargin-0.02, 1-tMargin+0.018, "Moriond EWK 2017");
  label.DrawLatex(1-rMargin, 1-tMargin+0.018, "Moriond 2017");

  //  label.SetTextFont(41); label.SetTextSize(0.6*tMargin);
  //  label.DrawLatex(1-rMargin-0.04,1-tMargin-0.07,"13 TeV");

  TLatex label2; label2.SetNDC(); label2.SetTextAlign(31);// label2.SetTextFont(41);                                                                                                                                                         
  label2.SetTextSize(0.6*tMargin);
  label2.SetTextFont(42);
  label2.DrawLatex(1-rMargin-0.04,1-tMargin-0.07,"35.9 fb^{-1} (13 TeV)");
...
TH2D baseHistogram(float Xmin, float Xmax, float Ymin, float Ymax){
  ...
  hbase.GetXaxis()->SetTitle("m#kern[0.12]{_{#lower[-0.12]{#tilde{t}}}} [GeV]");
